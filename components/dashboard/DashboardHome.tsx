...
import { CalendarDays, CreditCard, Receipt, TrendingUp, Users } from 'lucide-react';
        ...
        <div className="space-y-8 animate-in fade-in duration-700">
            <div className="flex items-center justify-between space-y-2">
                ...
                <div className="flex items-center space-x-2">
                    <Button variant="default" className="bg-primary text-primary-foreground hover:bg-amber-600 shadow-lg shadow-primary/20 transition-all hover:scale-105" onClick={() => router.push('/pos')}>
                        Launch POS Station
                    </Button>
                </div>
            </div>
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-2">
                <Card className="bg-card border-l-4 border-l-primary shadow-md hover:shadow-lg transition-shadow">
                    ...
                    <CardContent>
                        <div className="text-2xl font-bold text-foreground">IDR {stats.totalRevenue.toLocaleString('id-ID')}</div>
                        <p className="text-xs text-muted-foreground mt-1">
                            <span className={Number(stats.revenueGrowth) > 0 ? "text-green-500 font-bold" : "text-destructive"}>
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-7">
                <Card className="col-span-4 bg-card border-border shadow-lg">
                    <CardHeader>
                        <CardTitle className="text-foreground">Weekly Revenue Overview</CardTitle>
                    </CardHeader>
                    <CardContent className="pl-2">
                        <ResponsiveContainer width="100%" height={350}>
                            <BarChart data={chartData}>
                                ...
                                />
                                <Tooltip
                                    contentStyle={{ backgroundColor: '#18181b', border: '1px solid #27272a', borderRadius: '8px', color: '#f59e0b' }}
                        ...
                        <div className="space-y-8">
                            {recentActivity.length === 0 ? (
                                ...
                                recentActivity.map((tx) => (
                                    <div className="flex items-center group cursor-default" key={tx.id}>
                                        ...
                                        <div className="ml-4 space-y-1">
                                            <p className="text-sm font-medium leading-none text-foreground">{tx.barberName}</p>
                                            <p className="text-sm text-muted-foreground truncate max-w-[150px]">{tx.serviceName}</p>
                                        </div>
                                        <div className="ml-auto font-mono font-medium text-emerald-500 bg-emerald-950/30 px-2 py-0.5 rounded text-xs border border-emerald-900/50">+IDR {tx.amount.toLocaleString('id-ID')}</div>
                                    </div>
                                ))
                            )}
                        </div>
